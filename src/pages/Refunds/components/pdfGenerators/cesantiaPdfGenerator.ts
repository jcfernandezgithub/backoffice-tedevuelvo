...
export interface CesantiaCertificateData {
  correlativo: string
...
/**
 * Construye el certificado de cesantía (Southbridge - Póliza 0020123902).
 *
 * Lógica idéntica a la usada en `GenerateCesantiaCertificateDialog.tsx` —
 * extraída a un módulo puro para que el flujo masivo y el individual
 * compartan exactamente el mismo formato.
 */
export async function buildCesantiaPdf(
  refund: RefundRequest,
  formData: CesantiaCertificateData,
  ...
  const calculatePrimaNeta = (): number => {
    ...
    return Math.round(montoCredito * (TASA_CESANTIA_BRUTA / 100) * plazoMeses)
  }
...
/**
 * Construye un `CesantiaCertificateData` autocompletando desde el refund/snapshot
 * lo que sea posible. Los campos sin información quedan en blanco (válido para
 * batch: el certificado sigue siendo legítimo, sólo faltan datos del ejecutivo
 * y estado civil/región que se completan a mano en el flujo individual).
 */
export function buildCesantiaFormFromRefund(
  refund: RefundRequest,
  folio: string,
  overrides: Partial<CesantiaCertificateData> = {},
): CesantiaCertificateData {
  ...
  const apellidoPaterno = nameParts.length >= 3
    ? nameParts[nameParts.length - 2]
    : (nameParts.length === 2 ? nameParts[1] : '')
  const nombres = nameParts.length >= 3
    ? nameParts.slice(0, nameParts.length - 2).join(' ')
    : (nameParts[0] || '')

  const fechaNacimiento = (() => {
    const raw = snap.birthDate || refund.birthdate
    if (!raw) return ''
