...
const STAGE_GRADIENTS: Record<string, string> = {
  qualifying:        'from-amber-500 to-amber-600',
...
export default function AjustesPage() {
  const { objectives, save, reset, totalObjetivo } = useStageObjectives();
    ...
    <div className="p-6 max-w-3xl mx-auto space-y-8">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Configuración general del backoffice TeDevuelvo.
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-start justify-between gap-4">
            ...
            {isDirty && (
              <Badge variant="outline" className="bg-amber-50 border-amber-300 text-amber-700 whitespace-nowrap shrink-0">
                Cambios sin guardar
              </Badge>
            )}
          </div>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {changed && (
                        <p className="text-xs text-muted-foreground">
                          Valor por defecto: {defaultDays}d
                        </p>
                      )}
                    </div>
